...
class ProfileQueue {
   ...
   *
   * @note Pay attention to the nomenclature in here. The `free` queues are for
   * queues that have pointers to profiles ready to be used with new scan data
   * The `ready` queues have pointers to profiles that hold scan data that is
   * ready & waiting to be read out by the user.
   *
   * @note It is expected that the profile memory is recycled here. Profiles
   * read from the `ready` queue, after the contents have been copied over to
   * the user, are expected to be placed back into the `free` queue.
   */
 ...
 private:
  ...
  /// Maximum amount of pointers to profile that can be placed into a queue
  const uint32_t kMaxProfilesQueue = JS_SCAN_HEAD_PROFILES_MAX;

  ScanHeadModel &m_model;
  /// Will hold memory for all the profiles
  ...
  /// Common function to add a profile to a queue
  ...
  /// Common function to remove a profile from a queue
  ...
  /// Common function to peak at the current sequence number at the front
  ...
  /// Common function to get the approx number of profiles queued
 ...
 public:
  /// Struct used to report the status of the profile queues for frame scanning
  struct Report {
    uint32_t size_min;
    uint32_t size_max;
  ...
  ProfileQueue(ScanHeadModel &model) :
    m_model(model),
    m_single_queue(kMaxProfilesQueue),
