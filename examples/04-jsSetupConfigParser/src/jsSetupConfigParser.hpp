...
/// Internal function
jsCamera _Str2Cam(std::string str)
{
...
/// Internal function
jsLaser _Str2Las(std::string str)
{
...
/// Internal function
jsScanHeadType _Str2Type(std::string str)
{
...
/// Internal function
jsCableOrientation _Str2Cable(std::string str)
{
...
/// Internal function
std::string _Cam2Str(jsCamera camera)
{
...
/// Internal function
std::string _Las2Str(jsLaser laser)
{
  if (JS_LASER_1 == laser) {
    return "Laser1";
...
/// Internal function
std::string _Type2Str(jsScanHeadType type)
{
...
bool _IsLasHead(jsScanHeadType type)
{
  if ((JS_SCAN_HEAD_JS50X6B20 == type) ||
      (JS_SCAN_HEAD_JS50X6B30 == type) ||
      (JS_SCAN_HEAD_JS50Z820 == type) ||
...
/// Internal function
void _DummyLogger(jsError err, std::string msg) 
{
 ...
 *
 * @param json_path The complete path to the JSON file to parse.
  ...
  Logger *logger=nullptr)
{
  ...
  try {
    // read a JSON file
    logger(JS_ERROR_NONE, "Opening jsSetup config file: " + json_path);
    ...
    std::string units_str = jroot["Units"];
    ...
    for (json::iterator it = jsubroot.begin(); it != jsubroot.end(); ++it) {
      ...
      logger(JS_ERROR_NONE,
             "Creating scan head " + head_str + ", ID " + std::to_string(id));
      ...
      logger(JS_ERROR_NONE,
             head_str + " Laser on time: " +
             std::to_string(cfg.laser_on_time_min_us) + "," +
             std::to_string(cfg.laser_on_time_def_us) + "," +
             std::to_string(cfg.laser_on_time_max_us));
      logger(JS_ERROR_NONE,
             head_str + " Camera exposure time: " +
             std::to_string(cfg.camera_exposure_time_min_us) + "," +
             std::to_string(cfg.camera_exposure_time_def_us) + "," +
             std::to_string(cfg.camera_exposure_time_max_us));
      logger(JS_ERROR_NONE,
             head_str + " Laser detection threshold: " +
             std::to_string(cfg.laser_detection_threshold));
      logger(JS_ERROR_NONE,
             head_str + " Saturation threshold: " +
             std::to_string(cfg.saturation_threshold));
      logger(JS_ERROR_NONE,
             head_str + " Saturation percentage: " +
             std::to_string(cfg.saturation_percentage));
      ...
      for (json::iterator itc = jchild.begin(); itc != jchild.end(); ++itc) {
        ...
        logger(JS_ERROR_NONE,
               head_str + " Alignment " + pair_str +
               ": roll " + std::to_string(roll) +
               ", shift X " + std::to_string(shiftx) +
               ", shift Y " + std::to_string(shifty));
      ...
      for (json::iterator itc = jchild.begin(); itc != jchild.end(); ++itc) {
        ...
        for (json::iterator itv = jwin.begin(); itv != jwin.end(); ++itv) {
          ...
          vertices_str += " (" + std::to_string(v.x) +
                          "," + std::to_string(v.y) + ")";
        }
    ...
    std::sort(
      heads.begin( ),
      heads.end(),
      [](const jsScanHead &lhs, const jsScanHead &rhs)
      {
    ...
    jsubroot = jroot["PhaseTableEntries"];
    for (json::iterator it = jsubroot.begin(); it != jsubroot.end(); ++it) {
    ...
    for (auto &pc : phase_configs) {
      ...
      if ((int32_t) phase > current_phase) {
        ...
        std::string info =
          _Type2Str(pc.type) + " " + std::to_string(pc.serial) + " " +
          _Cam2Str(pc.camera) + " " + _Las2Str(pc.laser);

        logger(JS_ERROR_NONE,
               "Inserting " + info + " into phase " + std::to_string(phase));
